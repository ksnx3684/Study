...
const int SIZE = 8;
int sumArr(int*, int n = 1); // sumArr함수의 매개변수에 int n = 1로 디폴트값을 지정해준다. (디폴트 매개변수)

inline float square(float x) { return x * x; } // 일반적인 함수명 앞에 inline을 적어 인라인 함수를 호출해 준다. (인라인 함수)
